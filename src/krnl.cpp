...
/*==================================================== OVERLAP ENLARGEMENT FUNCTIONS ===============================================================*/

struct overlapEnlargementPair
{
    int index;
...
/*==================================================== AREA ENLARGEMENT FUNCTIONS ===============================================================*/

struct areaEnlargementPair
{
    int index;
    int areaEnlargement;
};
...
void sortByOverlapEnlargement(Node newNode)
{
    ...
    while (!isEmpty())
    {
        ...
        {
            // the node being compared to new node is leaf
            maxX = std::min(currentNode.maxX, newNode.maxX);
            maxY = std::min(currentNode.maxY, newNode.maxY);
            minX = std::max(currentNode.minX, newNode.minX);
            minY = std::max(currentNode.minY, newNode.minY);
        ...
        else
        {
            ...
            maxX = std::min(currentNode.LmaxX, newNode.maxX);
            maxY = std::min(currentNode.LmaxY, newNode.maxY);
            minX = std::max(currentNode.LminX, newNode.minX);
            minY = std::max(currentNode.LminY, newNode.minY);
            int leftOverlap = (maxX - minX) * (maxY - minY);
            std::cout << "Left Overlap: " << leftOverlap << " Node: " << nodeIndex << std::endl;

            // overlap of right child
            maxX = std::min(currentNode.RmaxX, newNode.maxX);
            maxY = std::min(currentNode.RmaxY, newNode.maxY);
            minX = std::max(currentNode.RminX, newNode.minX);
            minY = std::max(currentNode.RminY, newNode.minY);
...
#pragma HLS INTERFACE mode = m_axi port = output
    ...
    nodes[0] = Node(false, 0, 9, 0, 9, 10, 19, 10, 19, 1, 2);
